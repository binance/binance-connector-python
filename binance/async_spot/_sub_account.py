...
async def sub_account_futures_asset_transfer_history(
    ...
    Keyword Args:
        startTime (int, optional): Default return await the history with in 100 days
        endTime (int, optional): Default return await the history with in 100 days
...
async def sub_account_universal_transfer_history(self, **kwargs):
    ...
    Return await fromEmail equal master account email by default.
...
async def sub_account_update_ip_restriction(
...
async def managed_sub_account_investor_trans_log(
...
async def managed_sub_account_trading_trans_log(
    self, email: str, startTime: int, endTime: int, page: int, limit: int, **kwargs
):
...
async def managed_sub_account_deposit_address(self, email: str, coin: str, **kwargs):
