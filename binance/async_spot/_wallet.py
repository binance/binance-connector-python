...
async def convertible_coins(self, **kwargs):
...
async def toggle_auto_convertion(self, coin: str, enable: bool, **kwargs):
...
async def cloud_mining_trans_history(self, startTime: int, endTime: int, **kwargs):
...
async def convert_transfer(
    ...
    return await self.sign_request("POST", url_path, payload)


async def convert_history(self, startTime: int, endTime: int, **kwargs):
