...
def test_margin_interest_rate_history_without_asset():
    ...
    client.margin_interest_rate_history.when.called_with(
        '').should.throw(ParameterRequiredError)


@mock_http_response(responses.GET, '/sapi/v1/margin/interestRateHistory\\?' + urlencode(params), mock_item, 200)
