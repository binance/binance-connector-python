...
def test_cancel_margin_order_without_symbol():
    ...
    client.cancel_margin_order.when.called_with(
        '').should.throw(ParameterRequiredError)


@mock_http_response(responses.DELETE, '/sapi/v1/margin/order\\?symbol=ETHBTC&orderId=', mock_exception, 400)
def test_cancel_margin_order_without_order_id():
    """ Tests the API endpoint to cancel margin order without provide order id """

    client = Client(key, secret)
    client.cancel_margin_order.when.called_with(
        'ETHBTC', orderId='').should.throw(ClientError)


@mock_http_response(responses.DELETE, '/sapi/v1/margin/order\\?symbol=ETHBTC&orderId=' + orderId, mock_item, 200)
...
@mock_http_response(responses.DELETE, '/sapi/v1/margin/order\\?' + urlencode(params), mock_item, 200)
