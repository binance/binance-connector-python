...
@mock_http_response(responses.GET, '/sapi/v1/margin/maxTransferable', mock_exception, 400)
def test_margin_max_transferable_without_asset():
    """ Tests the API endpoint to query margin max transferable without symbol """

    client = Client(key, secret)
    client.margin_max_transferable.when.called_with(
        '').should.throw(ParameterRequiredError)


@mock_http_response(responses.GET, '/sapi/v1/margin/maxBorrowable\\?' + urlencode(params), mock_item, 200)
