...
def test_savings_flexible_redeem_without_productId():
    ...
    client.savings_flexible_redeem.when.called_with(
        '', 10, 'NORMAL').should.throw(ParameterRequiredError)
...
def test_savings_flexible_redeem_without_amount():
    ...
    client = Client(key, secret)
    client.savings_flexible_redeem.when.called_with(
        '1', '', 'NORMAL').should.throw(ParameterRequiredError)
...
def test_savings_flexible_redeem_without_type():
    ...
    client = Client(key, secret)
    client.savings_flexible_redeem.when.called_with(
        '1', '10', '').should.throw(ParameterRequiredError)


@mock_http_response(responses.POST, '/sapi/v1/lending/daily/redeem\\?productId=1&amount=2&type=NORMAL', mock_item, 200)
