...
def test_close_isolated_margin_listen_key_without_symbol():
    ...
    client.close_isolated_margin_listen_key.when.called_with(**param).should.throw(ParameterRequiredError)
...
@mock_http_response(responses.DELETE, '/sapi/v1/userDataStream/isolated\\?listenKey='+ listen_key + '&symbol=BTCUSDT', mock_item, 200)
def test_close_isolated_margin_listen_key():
    """ Tests the API endpoint to close an isolated margin listen key """

    client = Client(key)
    response = client.close_isolated_margin_listen_key(symbol='BTCUSDT', listenKey=listen_key)
