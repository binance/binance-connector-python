...
def test_new_listen_key_without_key():
    ...
    client = Client(key)
    client.renew_margin_listen_key.when.called_with(
        '').should.throw(ParameterRequiredError)


@mock_http_response(responses.PUT, '/sapi/v1/userDataStream\\?listenKey=' + listen_key, mock_exception, 400)
...
@mock_http_response(responses.PUT, '/sapi/v1/userDataStream\\?listenKey=' + listen_key, mock_item, 200)
