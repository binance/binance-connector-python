...
def test_user_universal_transfer_with_missing_field(params):
    ...
    client.user_universal_transfer.when.called_with(
...
def test_user_universal_transfer_with_invalid_enum_string():
    ...
    client = Client(key, secret)
    client.user_universal_transfer.when.called_with(
        **invalid_params).should.throw(ParameterValueError)


@mock_http_response(responses.POST, '/sapi/v1/asset/transfer\\?' + encoded_string(complete_params), mock_item, 200)
