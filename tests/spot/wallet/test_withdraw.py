...
def test_withdraw_without_coin():
    ...
    client.withdraw.when.called_with(
        '', 1, 'address').should.throw(ParameterRequiredError)
...
def test_withdraw_without_amount():
    ...
    client = Client(key, secret)
    client.withdraw.when.called_with(
        'BNB', '', 'address').should.throw(ParameterRequiredError)
...
def test_withdraw_without_address():
    ...
    client = Client(key, secret)
    client.withdraw.when.called_with(
        'BNB', 1, '').should.throw(ParameterRequiredError)


@mock_http_response(responses.POST, '/sapi/v1/capital/withdraw/apply\\?' + urlencode(params), mock_item, 200)
def test_withdraw():
