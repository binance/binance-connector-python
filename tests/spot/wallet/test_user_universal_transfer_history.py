...
def test_user_universal_transfer_with_missing_field():
    ...
    client.user_universal_transfer_history.when.called_with(
...
def test_user_universal_transfer_with_invalid_enum_string():
    ...
    client = Client(key, secret)
    client.user_universal_transfer_history.when.called_with(
        random_str()).should.throw(ParameterValueError)


@mock_http_response(responses.GET, '/sapi/v1/asset/transfer\\?type=UMFUTURE_MAIN', mock_item, 200)
