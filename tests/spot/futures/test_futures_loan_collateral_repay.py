...
def test_futures_loan_collateral_repay_without_quoteId():
    ...
    client.futures_loan_collateral_repay.when.called_with(**params).should.throw(ParameterRequiredError)


@mock_http_response(responses.POST, '/sapi/v1/futures/loan/collateralRepay\\?' + urlencode(params), mock_item, 200)
