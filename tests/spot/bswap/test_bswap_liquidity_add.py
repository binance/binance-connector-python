...
def test_bswap_liquidity_add_without_poolId():
    ...
    client.bswap_liquidity_add.when.called_with('', 'BUSD', '1').should.throw(ParameterRequiredError)
...
def test_bswap_liquidity_add_without_asset():
    ...
    client = Client(key, secret)
    client.bswap_liquidity_add.when.called_with('2', '', '1').should.throw(ParameterRequiredError)
...
def test_bswap_liquidity_add_without_quantity():
    ...
    client = Client(key, secret)
    client.bswap_liquidity_add.when.called_with('2', 'BUSD', '').should.throw(ParameterRequiredError)


@mock_http_response(responses.POST, '/sapi/v1/bswap/liquidityAdd\\?poolId=2&asset=BUSD&quantity=1', mock_item, 200)
