...
def test_bswap_liquidity_remove_without_poolId():
    ...
    client.bswap_liquidity_remove.when.called_with('', 'SINGLE', ['BUSD'], '12415').should.throw(ParameterRequiredError)
...
def test_bswap_liquidity_remove_without_type():
    ...
    client = Client(key, secret)
    client.bswap_liquidity_remove.when.called_with('2', '', ['BUSD'], '12415').should.throw(ParameterRequiredError)
...
def test_bswap_liquidity_remove_without_asset():
    ...
    client = Client(key, secret)
    client.bswap_liquidity_remove.when.called_with('2', 'SINGLE', None, '12415').should.throw(ParameterRequiredError)
...
def test_bswap_liquidity_remove_without_shareAmount():
    ...
    client = Client(key, secret)
    client.bswap_liquidity_remove.when.called_with('2', 'SINGLE', ['BUSD'], '').should.throw(ParameterRequiredError)
