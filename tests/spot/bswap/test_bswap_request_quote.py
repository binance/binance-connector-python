...
def test_bswap_request_quote_without_quoteAsset():
    ...
    client.bswap_request_quote.when.called_with('', 'BUSD', '30000').should.throw(ParameterRequiredError)
...
def test_bswap_request_quote_without_baseAsset():
    ...
    client = Client(key, secret)
    client.bswap_request_quote.when.called_with('USDT', '', '30000').should.throw(ParameterRequiredError)
...
def test_bswap_request_quote_without_quoteQty():
    ...
    client = Client(key, secret)
    client.bswap_request_quote.when.called_with('USDT', 'BUSD', None).should.throw(ParameterRequiredError)
