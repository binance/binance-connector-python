...
def test_sub_account_api_add_ip_with_missing_param(params):
    """Tests the API endpoint to add IP for a sub-account API key with missing param"""

    client.sub_account_api_add_ip.when.called_with(**params).should.throw(
...
def test_sub_account_api_add_ip():
    """Tests the API endpoint to add IP for a sub-account API key"""

    client.sub_account_api_add_ip(**complete_params).should.equal(mock_item)
