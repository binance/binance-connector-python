...
def test_sub_account_enable_futures_without_email():
    ...
    client.sub_account_enable_futures.when.called_with(
        '').should.throw(ParameterRequiredError)


@mock_http_response(responses.POST, '/sapi/v1/sub-account/futures/enable\\?email=alice@test.com', mock_item, 200)
