...
def test_sub_account_futures_account_v2_without_email():
    ...
    client.sub_account_futures_account_v2.when.called_with(**params).should.throw(ParameterRequiredError)
...
@mock_http_response(responses.GET, '/sapi/v2/sub-account/futures/account\\?' + encoded_string(params), mock_item, 200)
def test_sub_account_futures_account_v2():
    """ Tests the API endpoint to get sub account futures account """

    client = Client(key, secret)
    response = client.sub_account_futures_account_v2(**params)
