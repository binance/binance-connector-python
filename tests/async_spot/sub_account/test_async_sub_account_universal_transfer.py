...
@pytest.mark.asyncio
async def test_sub_account_universal_transfer_without_fromAccountType():
    ...
    try:
        response = await client.sub_account_universal_transfer(**params)
...
@pytest.mark.asyncio
async def test_sub_account_universal_transfer_without_toAccountType():
...
@pytest.mark.asyncio
async def test_sub_account_universal_transfer_without_asset():
...
@pytest.mark.asyncio
async def test_sub_account_universal_transfer_without_amount():
...
@mock_async_http_response(
    "POST",
    "/sapi/v1/sub-account/universalTransfer\\?" + encoded_string(params, True),
    mock_item,
    200,
)
async def test_sub_account_universal_transfer():
