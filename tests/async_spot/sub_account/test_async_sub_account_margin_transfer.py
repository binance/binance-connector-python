...
@pytest.mark.asyncio
async def test_sub_account_margin_transfer_without_email():
    ...
    try:
        response = await client.sub_account_margin_transfer(**params)
...
@pytest.mark.asyncio
async def test_sub_account_margin_transfer_without_asset():
...
@pytest.mark.asyncio
async def test_sub_account_margin_transfer_without_amount():
...
@pytest.mark.asyncio
async def test_sub_account_margin_transfer_without_type():
...
@mock_async_http_response(
    "POST",
    "/sapi/v1/sub-account/margin/transfer\\?" + encoded_string(params, True),
    mock_item,
    200,
)
async def test_sub_account_margin_transfer():
