...
async def test_managed_sub_account_investor_trans_log_with_missing_parameters(params):
    ...
    with pytest.raises(ParameterRequiredError):
        await client.managed_sub_account_investor_trans_log(
        **params
    )
...
@mock_async_http_response(
    "GET",
    "/sapi/v1/managed-subaccount/queryTransLogForInvestor\\?" + encoded_string(params, True),
    mock_item,
    200,
)
async def test_managed_sub_account_investor_trans_log():
    """Tests the API endpoint to query managed sub account transfer log"""

    client = Client(key, secret)
    response = await client.managed_sub_account_investor_trans_log(**params)
