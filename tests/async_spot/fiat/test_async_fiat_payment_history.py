...
@pytest.mark.asyncio
async def test_fiat_payment_history_without_type():
    """Tests the API endpoint to get fiat order history"""

    client = Client(key, secret)
    try:
        response = await client.fiat_payment_history("")
...
async def test_fiat_payment_history():
    """Tests the API endpoint to get fiat payments history"""

    client = Client(key, secret)
    response = await client.fiat_payment_history(**params)
