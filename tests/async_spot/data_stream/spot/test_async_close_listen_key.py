...
@pytest.mark.asyncio
async def test_close_listen_key_without_key():
    """Tests the API endpoint to close listen key without the key"""

    client = Client(key)
    try:
        response = await client.close_listen_key("")
...
async def test_close_listen_key_with_wrong_key():
    """Tests the API endpoint to close listen key with wrong key"""

    client = Client(key)
    try:
        response = await client.close_listen_key(listen_key)
    except Exception as e:
...
@pytest.mark.asyncio
@mock_async_http_response(
    "DELETE", "/api/v3/userDataStream\\?listenKey=" + listen_key, mock_item, 200
)
async def test_close_listen_key():
