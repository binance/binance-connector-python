...
@pytest.mark.asyncio
async def test_post_an_oco_order_without_param():
    """Tests the API endpoint to post a new oco order without parameters"""

    client = Client(key, secret)
    with pytest.raises(ParameterRequiredError):
        await client.new_oco_order("", "", "", "", "")


@pytest.mark.asyncio
@mock_async_http_response(
    "POST", "/api/v3/order/oco\\?" + urlencode(params), mock_item, 200
)
async def test_post_an_oco_order():
