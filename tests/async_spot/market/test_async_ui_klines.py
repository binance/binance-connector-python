...
async def test_kline_without_symbol():
    ...
    try:
        response = await client.ui_klines(symbol="", interval="1m")
...
async def test_kline_without_interval():
    ...
    try:
        response = await client.ui_klines(symbol="BTCUSDT", interval="")
    except Exception as e:
        assert isinstance(e, ParameterRequiredError)
    else:
        assert isinstance(response, ParameterRequiredError)
...
@mock_async_http_response(
    "GET", "/api/v3/uiKlines\\?symbol=BTCUSDT&interval=1h", mock_item, 200
)
async def test_kline_with_default_limit():
    """Tests the API endpoint to get kline with default limit"""

    response = await client.ui_klines(symbol="BTCUSDT", interval="1h")
...
async def test_kline_with_given_params():
    ...
    response = await client.ui_klines(
