...
@mock_http_response(responses.DELETE, '/api/v3/orderList\\?symbol=ETHBTC&orderListId=', mock_exception, 400)
def test_cancel_oco_order_without_orderListId():
    """ Tests the API endpoint to cancel order without provide orderListId """

    client =  binance.Trade(key, secret)
    client.cancel_oco_order.when.called_with('ETHBTC', orderListId='').should.throw(APIException)

@mock_http_response(responses.DELETE, '/api/v3/orderList\\?' + urlencode(params), mock_item, 200)
