...
def subscribe_flexible_product():
    try:
        response = client.rest_api.subscribe_flexible_product(
            product_id="1",
            amount=1.0,
        )

        rate_limits = response.rate_limits
        logging.info(f"subscribe_flexible_product() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"subscribe_flexible_product() response: {data}")
    except Exception as e:
        logging.error(f"subscribe_flexible_product() error: {e}")
