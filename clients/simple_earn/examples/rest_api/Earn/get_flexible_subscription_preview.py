...
def get_flexible_subscription_preview():
    try:
        response = client.rest_api.get_flexible_subscription_preview(
            product_id="1",
            amount=1.0,
        )

        rate_limits = response.rate_limits
        logging.info(f"get_flexible_subscription_preview() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_flexible_subscription_preview() response: {data}")
    except Exception as e:
        logging.error(f"get_flexible_subscription_preview() error: {e}")
