...
def get_rwusd_subscription_history():
    try:
        response = client.rest_api.get_rwusd_subscription_history()

        rate_limits = response.rate_limits
        logging.info(f"get_rwusd_subscription_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_rwusd_subscription_history() response: {data}")
    except Exception as e:
        logging.error(f"get_rwusd_subscription_history() error: {e}")
