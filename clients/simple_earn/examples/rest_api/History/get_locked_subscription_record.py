...
def get_locked_subscription_record():
    try:
        response = client.rest_api.get_locked_subscription_record()

        rate_limits = response.rate_limits
        logging.info(f"get_locked_subscription_record() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_locked_subscription_record() response: {data}")
    except Exception as e:
        logging.error(f"get_locked_subscription_record() error: {e}")
