...
def get_nft_withdraw_history():
    try:
        response = client.rest_api.get_nft_withdraw_history()

        rate_limits = response.rate_limits
        logging.info(f"get_nft_withdraw_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_nft_withdraw_history() response: {data}")
    except Exception as e:
        logging.error(f"get_nft_withdraw_history() error: {e}")
