...
def search_rwa_token():
    try:
        response = client.rest_api.search_rwa_token(
            keyword="NVDA",
        )

        rate_limits = response.rate_limits
        logging.info(f"search_rwa_token() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"search_rwa_token() response: {data}")
    except Exception as e:
        logging.error(f"search_rwa_token() error: {e}")
