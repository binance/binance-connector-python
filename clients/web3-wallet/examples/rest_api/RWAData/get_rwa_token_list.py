...
def get_rwa_token_list():
    try:
        response = client.rest_api.get_rwa_token_list()

        rate_limits = response.rate_limits
        logging.info(f"get_rwa_token_list() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_rwa_token_list() response: {data}")
    except Exception as e:
        logging.error(f"get_rwa_token_list() error: {e}")
