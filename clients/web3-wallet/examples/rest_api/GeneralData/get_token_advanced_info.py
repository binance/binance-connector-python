...
def get_token_advanced_info():
    try:
        response = client.rest_api.get_token_advanced_info(
            binance_chain_id="1",
            token_contract_address="0x6982508145454ce325ddbe47a25d4ec3d2311933",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_token_advanced_info() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_token_advanced_info() response: {data}")
    except Exception as e:
        logging.error(f"get_token_advanced_info() error: {e}")
