...
def get_supported_chains():
    try:
        response = client.rest_api.get_supported_chains()

        rate_limits = response.rate_limits
        logging.info(f"get_supported_chains() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_supported_chains() response: {data}")
    except Exception as e:
        logging.error(f"get_supported_chains() error: {e}")
