...
def get_holders_ranking():
    try:
        response = client.rest_api.get_holders_ranking(
            binance_chain_id="1",
            token_contract_address="0x6982508145454ce325ddbe47a25d4ec3d2311933",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_holders_ranking() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_holders_ranking() response: {data}")
    except Exception as e:
        logging.error(f"get_holders_ranking() error: {e}")
