...
def get_token_dev_info():
    try:
        response = client.rest_api.get_token_dev_info(
            binance_chain_id="CT_501",
            token_contract_address="EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_token_dev_info() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_token_dev_info() response: {data}")
    except Exception as e:
        logging.error(f"get_token_dev_info() error: {e}")
