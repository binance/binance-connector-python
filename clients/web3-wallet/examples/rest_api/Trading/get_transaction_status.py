...
def get_transaction_status():
    try:
        response = client.rest_api.get_transaction_status(
            binance_chain_id="56",
            tx_hash="0xabc123def4567890abc123def4567890abc123def4567890abc123def4567890",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_transaction_status() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_transaction_status() response: {data}")
    except Exception as e:
        logging.error(f"get_transaction_status() error: {e}")
