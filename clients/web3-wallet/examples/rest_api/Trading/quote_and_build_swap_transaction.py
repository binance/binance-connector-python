...
def quote_and_build_swap_transaction():
    try:
        response = client.rest_api.quote_and_build_swap_transaction(
            binance_chain_id="56",
            amount="1000000",
        ...
        logging.info(f"quote_and_build_swap_transaction() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"quote_and_build_swap_transaction() response: {data}")
    except Exception as e:
        logging.error(f"quote_and_build_swap_transaction() error: {e}")


if __name__ == "__main__":
    quote_and_build_swap_transaction()
