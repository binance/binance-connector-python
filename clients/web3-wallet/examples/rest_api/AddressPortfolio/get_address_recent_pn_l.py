...
def get_address_recent_pn_l():
    try:
        response = client.rest_api.get_address_recent_pn_l(
            binance_chain_id="1",
            wallet_address="0x28c6c06298d514db089934071355e5743bf21d60",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_address_recent_pn_l() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_address_recent_pn_l() response: {data}")
    except Exception as e:
        logging.error(f"get_address_recent_pn_l() error: {e}")


if __name__ == "__main__":
    get_address_recent_pn_l()
