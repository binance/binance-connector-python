...
def get_address_pn_l_for_specific_token():
    try:
        response = client.rest_api.get_address_pn_l_for_specific_token(
            binance_chain_id="1",
        ...
        logging.info(
            f"get_address_pn_l_for_specific_token() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"get_address_pn_l_for_specific_token() response: {data}")
    except Exception as e:
        logging.error(f"get_address_pn_l_for_specific_token() error: {e}")


if __name__ == "__main__":
    get_address_pn_l_for_specific_token()
