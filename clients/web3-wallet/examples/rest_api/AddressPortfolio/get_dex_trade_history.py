...
def get_dex_trade_history():
    try:
        response = client.rest_api.get_dex_trade_history(
            binance_chain_id="1",
            wallet_address="0x28c6c06298d514db089934071355e5743bf21d60",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_dex_trade_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_dex_trade_history() response: {data}")
    except Exception as e:
        logging.error(f"get_dex_trade_history() error: {e}")


if __name__ == "__main__":
    get_dex_trade_history()
