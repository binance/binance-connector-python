...
def get_broadcast_orders():
    try:
        response = client.rest_api.get_broadcast_orders(
            address="0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045",
            binance_chain_id="1",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_broadcast_orders() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_broadcast_orders() response: {data}")
    except Exception as e:
        logging.error(f"get_broadcast_orders() error: {e}")


if __name__ == "__main__":
    get_broadcast_orders()
