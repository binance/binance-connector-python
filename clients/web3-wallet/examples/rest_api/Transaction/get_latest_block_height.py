...
def get_latest_block_height():
    try:
        response = client.rest_api.get_latest_block_height(
            binance_chain_id="1",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_latest_block_height() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_latest_block_height() response: {data}")
    except Exception as e:
        logging.error(f"get_latest_block_height() error: {e}")
