...
def subscribe_sol_staking():
    try:
        response = client.rest_api.subscribe_sol_staking(
            amount=1.0,
        )

        rate_limits = response.rate_limits
        logging.info(f"subscribe_sol_staking() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"subscribe_sol_staking() response: {data}")
    except Exception as e:
        logging.error(f"subscribe_sol_staking() error: {e}")
