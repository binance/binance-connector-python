...
def get_otc_blocktrade_events():
    try:
        response = client.rest_api.get_otc_blocktrade_events()

        rate_limits = response.rate_limits
        logging.info(f"get_otc_blocktrade_events() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_otc_blocktrade_events() response: {data}")
    except Exception as e:
        logging.error(f"get_otc_blocktrade_events() error: {e}")


if __name__ == "__main__":
    get_otc_blocktrade_events()
