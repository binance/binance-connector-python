...
def preview_otc_blocktrade():
    try:
        response = client.rest_api.preview_otc_blocktrade(
            secret_token="a1b2c3d4-e5f6-7890-abcd-ef1234567890"
        )

        rate_limits = response.rate_limits
        logging.info(f"preview_otc_blocktrade() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"preview_otc_blocktrade() response: {data}")
    except Exception as e:
        logging.error(f"preview_otc_blocktrade() error: {e}")


if __name__ == "__main__":
    preview_otc_blocktrade()
