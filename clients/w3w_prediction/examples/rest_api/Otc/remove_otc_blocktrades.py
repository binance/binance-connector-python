...
def remove_otc_blocktrades():
    try:
        response = client.rest_api.remove_otc_blocktrades(order_ids=[""])

        rate_limits = response.rate_limits
        logging.info(f"remove_otc_blocktrades() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"remove_otc_blocktrades() response: {data}")
    except Exception as e:
        logging.error(f"remove_otc_blocktrades() error: {e}")


if __name__ == "__main__":
    remove_otc_blocktrades()
