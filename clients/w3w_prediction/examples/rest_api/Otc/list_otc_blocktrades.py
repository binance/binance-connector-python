...
def list_otc_blocktrades():
    try:
        response = client.rest_api.list_otc_blocktrades()

        rate_limits = response.rate_limits
        logging.info(f"list_otc_blocktrades() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"list_otc_blocktrades() response: {data}")
    except Exception as e:
        logging.error(f"list_otc_blocktrades() error: {e}")


if __name__ == "__main__":
    list_otc_blocktrades()
