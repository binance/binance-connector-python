        ...
        * One order can only be modfied for less than 10000 times
        ...
        1 on IP rate limit(x-mbx-used-weight-1m)
                ...
                Args:
                    ...
                    order_id (Optional[int]):
                    orig_client_order_id (Optional[str]):
                    price_match (Optional[ModifyOrderPriceMatchEnum]): only available for `LIMIT`/`STOP`/`TAKE_PROFIT` order; can be set to `OPPONENT`/ `OPPONENT_5`/ `OPPONENT_10`/ `OPPONENT_20`: /`QUEUE`/ `QUEUE_5`/ `QUEUE_10`/ `QUEUE_20`; Can't be passed together with `price`
                    recv_window (Optional[int]):
        ...
        * In Hedge Mode, cannot be used with `BUY` orders in `LONG` position side. and cannot be used with `SELL` orders in `SHORT` position side
        ...
        Weight: 0
                ...
                Args:
                    ...
                    type (NewOrderTypeEnum): `LIMIT`, `MARKET`, `STOP`, `STOP_MARKET`, `TAKE_PROFIT`, `TAKE_PROFIT_MARKET`, `TRAILING_STOP_MARKET`
                    id (Optional[str]): Unique WebSocket request ID.
                    ...
                    price_match (Optional[NewOrderPriceMatchEnum]): only available for `LIMIT`/`STOP`/`TAKE_PROFIT` order; can be set to `OPPONENT`/ `OPPONENT_5`/ `OPPONENT_10`/ `OPPONENT_20`: /`QUEUE`/ `QUEUE_5`/ `QUEUE_10`/ `QUEUE_20`; Can't be passed together with `price`
