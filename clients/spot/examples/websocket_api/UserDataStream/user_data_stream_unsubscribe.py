...
async def user_data_stream_unsubscribe():
    ...
    try:
        ...
        logging.info(f"user_data_stream_unsubscribe() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"user_data_stream_unsubscribe() response: {data}")
    except Exception as e:
        logging.error(f"user_data_stream_unsubscribe() error: {e}")
    finally:
        if connection:
...
if __name__ == "__main__":
    asyncio.run(user_data_stream_unsubscribe())
