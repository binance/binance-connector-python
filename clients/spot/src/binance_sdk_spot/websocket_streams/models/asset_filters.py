...
from binance_sdk_spot.websocket_streams.models.max_asset_filter import MaxAssetFilter
...
class AssetFilters(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: MaxAssetFilter
        if not isinstance(v, MaxAssetFilter):
            error_messages.append(
                f"Error! Input type `{type(v)}` is not `MaxAssetFilter`"
            )
        else:
            match += 1
        if match > 1:
            # more than 1 match
            raise ValueError(
                "Multiple matches found when setting `actual_instance` in AssetFilters with oneOf schemas: MaxAssetFilter. Details: "
                + ", ".join(error_messages)
            )
        elif match == 0:
            # no match
            raise ValueError(
                "No match found when setting `actual_instance` in AssetFilters with oneOf schemas: MaxAssetFilter. Details: "
                + ", ".join(error_messages)
            )
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        try:
            instance.actual_instance = MaxAssetFilter.from_json(json_str)
            match += 1
