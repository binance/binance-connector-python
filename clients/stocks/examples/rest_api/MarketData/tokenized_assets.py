...
def tokenized_assets():
    try:
        response = client.rest_api.tokenized_assets()

        rate_limits = response.rate_limits
        logging.info(f"tokenized_assets() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"tokenized_assets() response: {data}")
    except Exception as e:
        logging.error(f"tokenized_assets() error: {e}")
