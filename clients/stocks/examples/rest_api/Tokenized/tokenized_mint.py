...
def tokenized_mint():
    try:
        response = client.rest_api.tokenized_mint(
            underlying_asset="AAPL",
            underlying_asset_amount="1",
        )

        rate_limits = response.rate_limits
        logging.info(f"tokenized_mint() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"tokenized_mint() response: {data}")
    except Exception as e:
        logging.error(f"tokenized_mint() error: {e}")
