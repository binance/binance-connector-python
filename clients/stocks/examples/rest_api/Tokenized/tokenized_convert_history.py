...
def tokenized_convert_history():
    try:
        response = client.rest_api.tokenized_convert_history()

        rate_limits = response.rate_limits
        logging.info(f"tokenized_convert_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"tokenized_convert_history() response: {data}")
    except Exception as e:
        logging.error(f"tokenized_convert_history() error: {e}")
