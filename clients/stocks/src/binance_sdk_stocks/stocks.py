...
from .websocket_streams import StocksWebSocketStreams
...
class Stocks:
    """Stocks API that exposes REST, and Stream APIs in a single interface."""
    ...
    @property
    def rest_api(self) -> StocksRestAPI:
        if self._rest_api is None and self._rest_api_config:
            ...
            self._rest_api = StocksRestAPI(self._rest_api_config)
        return self._rest_api

    @property
    def websocket_streams(self) -> StocksWebSocketStreams:
        if self._websocket_streams is None and self._websocket_streams_config:
            ...
            self._websocket_streams = StocksWebSocketStreams(
                self._websocket_streams_config
            )
