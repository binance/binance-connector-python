...
class PlaceMultipleOrdersBatchOrdersParameterInner(BaseModel):
    ...
    good_till_date: Optional[StrictInt] = Field(
        default=None,
        description="Auto-cancel time for `GTD` orders.",
        alias="goodTillDate",
    )
    ...
    def self_trade_prevention_mode_validate_enum(cls, value):
        ...
        if value not in set(["NONE", "EXPIRE_TAKER", "EXPIRE_BOTH", "EXPIRE_MAKER"]):
            raise ValueError(
                "must be one of enum values ('NONE', 'EXPIRE_TAKER', 'EXPIRE_BOTH', 'EXPIRE_MAKER')"
            )
        return value
