...
def close_margin_user_data_stream():
    try:
        response = client.rest_api.close_margin_user_data_stream(
            listenkey="listenkey_example"
        )

        rate_limits = response.rate_limits
        logging.info(f"close_margin_user_data_stream() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"close_margin_user_data_stream() response: {data}")
    except Exception as e:
        logging.error(f"close_margin_user_data_stream() error: {e}")


if __name__ == "__main__":
    close_margin_user_data_stream()
