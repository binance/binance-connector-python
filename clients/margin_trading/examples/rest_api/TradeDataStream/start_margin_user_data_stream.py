...
def start_margin_user_data_stream():
    try:
        response = client.rest_api.start_margin_user_data_stream()

        rate_limits = response.rate_limits
        logging.info(f"start_margin_user_data_stream() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"start_margin_user_data_stream() response: {data}")
    except Exception as e:
        logging.error(f"start_margin_user_data_stream() error: {e}")
